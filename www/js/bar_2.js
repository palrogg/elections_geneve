(function () {
  // bypass cache for quick updates
  // $('#barTitle').text('Conseil d’Etat, résultats définitifs')
  ...
  var d = new Date();
  var targetFile = "data/data-ce-2.json?time=" + d.getHours() + '_' + d.getMinutes();
  ...
  d3.json(targetFile, function(error, data) {
      ...
      svg.append('line')
          ...
          .attr("x1", 20)
          .attr('y1', y('Luc Barthassat') + height / 20)
          .attr('x2', x(60000))
          .attr('y2', y('Luc Barthassat') + height / 20)
          .attr('stroke', '#000')
